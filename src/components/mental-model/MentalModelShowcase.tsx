...
export default function MentalModelShowcase({
  ...
  const orderedSections = useMemo(
    () =>
      model.section_order
        .map((sectionId) => outputSections.find((section) => section.id === sectionId))
        .filter((section): section is (typeof outputSections)[number] => Boolean(section)),
    [model.section_order],
  );
  ...
  const goToRelativeSection = (direction: -1 | 1) => {
    const nextIndex =
      (activeIndex + direction + orderedSections.length) % orderedSections.length;
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-5 font-display text-[2.2rem] font-extrabold leading-[1] text-text-primary md:text-[3rem]">
              See the generated mental model the product is designed to deliver.
            </h2>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-text-secondary md:text-base">
              This walkthrough uses a stored analysis of a Netflix engineering
              article so you can inspect the structure, pacing, and navigation
              of the intended output experience without placeholder filler.
            </p>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 truncate text-sm font-semibold text-text-primary">
                  netflixtechblog.com/scaling-archunit-with-nebula-archrules
                </p>
            ...
            <div className="flex flex-col gap-4 border-b-2 border-border-default pb-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-text-secondary">
                  Move through the generated model one lens at a time.
                </p>
              </div>
